...
export const getPackageDetailByTracking = async (req, res) => {
  try {
    const { tracking } = req.query;
    // console.log(tracking);

    // const packageDetail = await Package.findOne({ tracking: tracking });
    let aggregateQuery = [...initialQuery,
      {
...
export const getPackageListByStationAndDate = async (req, res) => {
  try {
    // sort should look like this: { "field": "userId", "sort": "desc"}
...
export const getSumPackagesForWhatsApp = async (req, res) => {
  try {
    const { station, search = "" } = req.query;
    var sumPackages = await Package.aggregate([...initialQuery,
      {
        $match: {
          $and: [
            { station: station },
    ...
    if (sumPackages.length !== 0) {
      ...
      for (let index = 0; index < sumPackages.length; index++) {
        ...
        const packages = await Package.aggregate([...initialQuery,
          {
            $match: {
              $and: [
                { station: station },
...
export const getSumPackagesForDelivered = async (req, res) => {
  try {
    const { station, search = "" } = req.query;
    console.log(station);
    var sumPackages = await Package.aggregate([...initialQuery,
